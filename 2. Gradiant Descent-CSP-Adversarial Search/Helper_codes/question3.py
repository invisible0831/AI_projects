...
class Board:
    ...
    def __init__(self, *args):
        ...
        if isinstance(args[0], list):
            ...
            p1 = Player(board.players[0].getCol(
            ), board.players[0].getX(), board.players[0].getY())
            p2 = Player(board.players[1].getCol(
            ), board.players[1].getX(), board.players[1].getY())
            self.players = [p1, p2]
    ...
    def move(self, nextPlace, playerColor):
        ...
        currentPlace = IntPair(
            self.players[playerColor-1].getX(), self.players[playerColor-1].getY())
        if ((nextPlace.x < 0) | (nextPlace.y < 0) | (nextPlace.x >= self.__size) | (nextPlace.y >= self.__size)):
            ...
            if (id == -1):
                if (self.__playerTerminalSourceId[playerColor-1] != -1):
                    self.__cells[nextPlace.x][nextPlace.y].setColor(
                        playerColor)
    ...
    def start(self, numberOfMatches):
        for k in range(numberOfMatches):
            ...
            while (self.__board.win() == 0):
                    ...
                    for i in range(0, 8):
                        for j in range(0, 8):
                            if ((self.__board.getCell(i, j).getId() != -1) and (self.__board.getCell(i, j).getColor() == 0)):
                                ...
                                m = Rectangle(Point(i+1, j+1), Point(i+2, j+2))
                                m.setFill(
                                    self.colors[self.__board.getCell(i, j).getColor()])
                                m.draw(win)
                ...
                self.__nextPlace = self.__players[self.__turn].getMove(
                    Board(self.__board))
                    ...
                    if (self.__board.getScore(1) > self.__board.getScore(2)):
                        if numberOfMatches == 1:
                            ...
                            print("score player 1: " +
                                  str(self.__board.getScore(1)))
                            print("score player 2: " +
                                  str(self.__board.getScore(2)) + "\n")
